...
def cmd(mycmd, env=None, debug=False, fail_func=None):
    ...
    if ret:
        if fail_func:
            ...
            fail_func()
        raise CatalystError("cmd(%r) exited %s" % (args, ret), print_traceback=False)
...
def file_check(filepath, extensions=None):
    ...
    files = [
        x
        for x in files
        if not x.endswith(".CONTENTS")
        and not x.endswith(".CONTENTS.gz")
        and not x.endswith(".DIGESTS")
        and not x.endswith(".sha256")
    ]
    if len(files) == 1:
        return files[0]
    if len(files) > 1:
        msg = (
            "Ambiguous Filename: %s\nPlease specify the correct extension as well"
            % filepath
        )
        ...
        if myfile not in settings:
            ...
            if settings[myfile][0] == "/":
                if not os.path.exists(settings[myfile]):
                    raise CatalystError(
                        "Cannot locate specified " + myfile + ": " + settings[myfile],
...
def read_makeconf(mymakeconffile):
    if os.path.exists(mymakeconffile):
        try:
            ...
            raise CatalystError(
                "Could not parse make.conf file " + mymakeconffile, print_traceback=True
            ) from e
    else:
        makeconf = {}
        return makeconf
