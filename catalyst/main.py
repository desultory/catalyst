        ...
        try:
            ...
            for key in config:
                if key not in valid_config_file_values:
                    log.critical(
                        "Unknown option '%s' in config file %s", key, config_file
                    )
            conf_values.update(config)
        except Exception as e:
            log.critical(
                "Could not find parse configuration file: %s: %s", config_file, e
            )
...
def build_target(addlargs):
    try:
        ...
        raise CatalystError(
            'Target "%s" not available.' % target, print_traceback=True
        ) from e
...
def get_parser():
    ...
    group.add_argument(
        "-F", "--fetchonly", default=False, action="store_true", help="fetch files only"
    )
    group.add_argument(
        "-c",
        "--configs",
    ...
    group.add_argument(
        "-s", "--snapshot", type=str, help="Make an ebuild repo snapshot"
    )
...
def trace(func, *args, **kwargs):
    ...
    try:
        ignoredirs.remove(os.path.dirname(os.path.dirname(os.path.realpath(__file__))))
    except KeyError:
        pass
...
def _main(parser, opts):
    ...
    if log_level is None:
        if opts.debug:
            ...
            log_level = "notice"
    log.setup_logging(
        log_level, output=opts.log_file, debug=opts.debug, color=opts.color
    )
